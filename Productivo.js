...
function (record, search, runtime, email, file, format, util, currency) {
    ...
    function getInputData() {
        ...
        try{
            ...
            if(!dataParams.error){
                ...
                log.debug(proceso, "GetInputData - LINE 75 - ssJournalsRunRange.length: "+ ssJournalsRunRange.length);
                ...
                log.debug(proceso, "GetInputData - LINE 82 - idJournals: "+JSON.stringify(idJournals));
                ...
                if (!isEmpty(idJournals) && idJournals.length > 0)
                {
                    ...
                    log.debug(proceso, "GetInputData - LINE 116 - ssTransactionsRunRange: "+ssTransactionsRunRange.length);
                    ...
                    log.debug(proceso, "GetInputData - LINE 172 - ssJournalsAdjustRunRange: "+ssJournalsAdjustRunRange.length);
                ...
                if (!isEmpty(dataProcesar) && dataProcesar.length > 0)
                {
                    for (i = 0; i < dataProcesar.length; i++)
                    {
                        for (j=0; j < idJournals.length; j++)
                        {
                            var object = {};
                            object.idCustomer = dataProcesar[i].idCustomer;
                            object.subsidiary = dataProcesar[i].subsidiary;
                            object.currency = dataProcesar[i].currency;
                            object.idPayment = dataProcesar[i].idPayment;
                            object.nroLocalizado = dataProcesar[i].nroLocalizado;
                            object.araccount = dataParams.araccount;
                            object.payMethod = dataParams.payMethod;
                            object.adjustPositiveBalance = dataParams.adjustPositiveBalance;
                            object.journalId = idJournals[j];
                            dataProcesarMap.push(object);
                        }
                    }
                }

                log.debug(proceso, 'dataProcesarMap: '+JSON.stringify(dataProcesarMap));

                return dataProcesarMap;
    ...
    function map(context) {
        ...
        try {
            ...
            log.debug(proceso, 'Map - LINE 246 - resultado: '+JSON.stringify(resultado));
            ...
            if (!isEmpty(resultado)) {
                ...
                log.debug(proceso, 'Map - LINE 268');
                var clave = obj.idPayment + obj.journalId;
    ...
    function reduce(context) {
        ...
        try {
            ...
            log.debug(proceso, 'Reduce - LINE 267 - context : ' + JSON.stringify(context));
            ...
            if (!isEmpty(context.values) && context.values.length > 0) {
                ...
                for (var k = 0; k < context.values.length; k++)
                {
                    ...
                    log.debug(proceso,'Reduce - LINE 275 - registro: '+JSON.stringify(registro));
                    ...
                    log.debug(proceso,'Reduce - LINE 337 - countLines'+sublist+': '+countLinesCredit);
                    ...
                    for (k = 0; k < countLinesCredit; k++)
                    {
                        ...
                        log.debug(proceso,'Reduce - LINE 351 - docNumber: '+docNumber+' - registro.idPayment: '+registro.idPayment);
                    ...
                    if (existeCredito && creditAmount > 0)
                    {
                        ...
                        log.debug(proceso,'Reduce - LINE 385 - countLines'+sublist+': '+countLinesApply);
                        ...
                        if (countLinesApply > 0)
                        {
                            for (j = 0; j < countLinesApply; j++)
                            {
                                ...
                                {
                                    log.debug(proceso,'Reduce - LINE 404 - docNumber: '+docNumber+' - registro.journalId: '+registro.journalId);
                                    if (registro.journalId == docNumber)
                                    {
                                        existeAsiento = true;
                            ...
                            log.debug(proceso,`Reduce - LINE 437 - registro: ${JSON.stringify(newRecord)}`);
                            ...
                            {
                                var mensaje = `No existen asientos disponibles para aplicar a la transaccion con ID: ${registro.idPayment} y numero de localizado: ${registro.nroLocalizado}`;
